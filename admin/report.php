...
<div class="col-lg-12">
	...
	<div class="row">
		...
		<div class="col-md-9">
			...
			<div>
			<h3 class="text-center"><b>GOUTHAMI INSTITUTE OF TECHNOLOGY AND MANAGEMENT FOR WOMEN</b></h3>
			<h3 class="text-center"><i>Faculty Evaluation Report</i></h3>
			<hr>
			<table width="100%">
					<tr>
					...
					<tr>
						...
						<td width="50%"><p><b>Overall Average Rating: <span id="overall-average" class="highlight-border"></span></b></p></td>
					</tr>
			</table>
			</div>
				<fieldset class="border border-info p-2 w-100">
				   <legend  class="w-auto">Rating Legend</legend>
				   <p>5 = Excellent, 4 = Very Good, 3 = Good, 2 = Average, 1 = Below Average</p>
				</fieldset>
				<?php 
							$q_arr = array();
...
<style>
	...
	.highlight-border {
        ...
        padding: 5px; /* Optional: Add padding to the highlighted element */
    	}
</style>
<noscript>
	<style>
					...
					if(Object.keys(resp).length <= 0){
						...
						Object.keys(data).map(q=>{
							Object.keys(data[q]).map(r=>{
								...
								var roundedValue = Math.round(data[q][r]) //round the percentage
        						$('.rate_' + r + '_' + q).text(roundedValue + '%') //display the percentages
							})
						})
						var overallAvg = 0;
        				var count = 0;
        				Object.keys(data).map(q => {
           				Object.keys(data[q]).map(r => {
                		overallAvg += parseInt(data[q][r]) * parseInt(r);
                		count += parseInt(data[q][r]);
            });
        });
        overallAvg = overallAvg / count;

        // Display the overall average
        $('#overall-average').text(overallAvg.toFixed(2));
					}
